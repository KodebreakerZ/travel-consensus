...
*/
var tripTable = pg.schema.createTableIfNotExists('trip', function (table) {
  table.increments('id').primary();
  table.string('name').notNullable().defaultTo('Unknown');
})

var usersTable = pg.schema.createTableIfNotExists('users', function (table) {
  table.increments('id').primary();
...
var trip_usersTable = pg.schema.createTableIfNotExists('trip_users', function (table) {
  table.increments('id').primary();
  table.integer('id_trip').references('id').inTable('trip');
  table.integer('id_user').references('id').inTable('users');
})

var taskTable = pg.schema.createTableIfNotExists('task', function (table) {
  table.increments('id').primary();
  table.string('name').notNullable();
...
var messageTable = pg.schema.createTableIfNotExists('message', function (table) {
  table.increments('id').primary();
...
var suggestionTable = pg.schema.createTableIfNotExists('suggestion', function (table) {
  table.increments('id').primary();
...
var tableCreators = [tripTable, usersTable, trip_usersTable,
										 taskTable, messageTable, suggestionTable];
...
.then(function() {
	return Bluebird.each(tableCreators, function(tableCreator) {
		console.log('creating table!', tableCreator);
		// tableCreator();
	})
})
.then(function(tables) {
	console.log('created all the tables!', tables)
})
.catch(function(error) {
